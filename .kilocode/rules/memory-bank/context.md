...
- [x] **Prevent Multiple Order Resume** - Added validation to prevent resuming a held order when there's already an active order with items. The Resume button is disabled and shows an alert if the user tries to resume when `currentOrder.items.length > 0`. |
