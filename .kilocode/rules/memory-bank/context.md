...
| 2026-05-02 | **HelOrder Decision Dialog** - Added workflow when selecting a customer with an existing held order. Presents modal with two options: "Resume Existing Order" loads the held items into the active session, or "Start New Order" preserves the held order and opens a fresh transaction. Prevents data loss and enables seamless switching between both options. Updated: `src/app/pos/page.tsx` with new state variables (`showHeldOrderDialog`, `pendingCustomerWithHeldOrder`, `heldOrderForCustomer`) and functions (`resumeExistingOrder`, `startNewOrder`). | | |
